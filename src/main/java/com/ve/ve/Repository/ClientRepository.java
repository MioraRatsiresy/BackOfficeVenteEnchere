...
public class ClientRepository implements ClientDAO {
    ...
    public void inscriptionClient(Client client) {
        String sql = "INSERT INTO client values (default,'" + client.getNom() + "','" + client.getPrenom() + "','"
                + client.getContact() + "','" + client.getIdentifiant() + "',md5('" + client.getPwd() + "'))";
    ...
    public void rechargerMonCompte(CompteClient compte) {
        String sql = "INSERT INTO compteclient values (default,'" + compte.getMontant() + "','" + compte.getEtat()
                + "','" + compte.getClientid() + "','" + compte.getActionTransaction() + "')";
        jdbcTemplate.update(sql);
